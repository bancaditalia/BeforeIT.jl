...
initial_conditions = matread(joinpath(dir,
                                      "../data/steady_state/initial_conditions/2010Q1.mat"))
...
Q_s_i, I_d_i, DM_d_i, N_d_i, Pi_e_i, DL_d_i, K_e_i, L_e_i, new_P_i = Bit.firms_expectations_and_decisions(model.firms,
                                                                                                          prop.tau_SIF,
                                                                                                          prop.tau_FIRM,
                                                                                                          prop.theta,
                                                                                                          prop.theta_DIV,
...
DL_i = Bit.search_and_matching_credit(DL_d_i, K_e_i, L_e_i, model.bank.E_k, prop.zeta,
                                      prop.zeta_LTV) # actual loans obtained
...
Bit.cons_inv_budget_w_act!(model.w_act,
                           prop.psi,
                           prop.psi_H,
                           prop.tau_VAT,
                           prop.tau_CF,
...
Bit.cons_inv_budget_w_inact!(model.w_inact,
                             prop.psi,
                             prop.psi_H,
                             prop.tau_VAT,
                             prop.tau_CF,
...
Bit.cons_inv_budget_fowner!(model.firms,
                            prop.psi,
                            prop.psi_H,
                            prop.tau_VAT,
                            prop.tau_CF,
...
Bit.cons_inv_budget_bowner!(model.bank,
                            prop.psi,
                            prop.psi_H,
                            prop.tau_VAT,
                            prop.tau_CF,
...
Bit.search_and_matching!(model.w_act,
                         model.w_inact,
                         model.firms,
                         model.gov,
                         model.bank,
...
model.agg.pi_[prop.T_prime + t], model.agg.P_bar = Bit.inflation_priceindex(model.firms.P_i,
                                                                            model.firms.Y_i,
                                                                            model.agg.P_bar)
...
model.w_act.Y_h .= Bit.households_income_act(model.w_act.w_h,
                                             model.w_act.O_h,
                                             prop.tau_SIW,
                                             prop.tau_INC,
                                             prop.theta_UB,
...
model.w_inact.Y_h .= Bit.income_w_inact(length(model.w_inact), model.gov.sb_inact,
                                        model.gov.sb_other, model.agg.P_bar_HH)

model.firms.Y_h .= Bit.income_fowner(model.firms.Pi_i,
                                     prop.tau_INC,
                                     prop.tau_FIRM,
                                     prop.theta_DIV,
                                     model.gov.sb_other,
...
model.bank.Y_h = Bit.income_bowner(model.bank.Pi_k,
                                   prop.tau_INC,
                                   prop.tau_FIRM,
                                   prop.theta_DIV,
                                   model.gov.sb_other,
...
model.w_act.D_h .+= Bit.new_deposits(model.w_act, prop.tau_VAT, prop.tau_CF, model.cb.r_bar,
                                     model.bank.r)
model.w_inact.D_h .+= Bit.new_deposits(model.w_inact, prop.tau_VAT, prop.tau_CF,
                                       model.cb.r_bar, model.bank.r)
model.firms.D_h .+= Bit.new_deposits(model.firms, prop.tau_VAT, prop.tau_CF, model.cb.r_bar,
                                     model.bank.r)
model.bank.D_h += Bit.new_deposits(model.bank, prop.tau_VAT, prop.tau_CF, model.cb.r_bar,
                                   model.bank.r)

# update central bank equity
model.cb.E_CB = Bit.central_bank_equity(model.cb.r_bar, model.bank.D_k, model.gov.L_G,
                                        model.cb.r_G)

# compute government revenues (Y_G), surplus/deficit (Pi_G) and debt (L_H)
Y_G = Bit.government_revenues(model.w_act,
                              model.w_inact,
                              model.firms,
                              model.bank,
                              model.rotw,
...
Pi_G = Bit.government_debt(model.gov, model.w_act, prop, Y_G, model.cb.r_G,
                           model.agg.P_bar_HH)

model.gov.L_G += Pi_G

# compute firms deposits, loans and equity
DD_i = Bit.new_deposits_firms(model.firms, DL_i, prop, model.bank.r, model.cb.r_bar,
                              model.agg.P_bar_HH)
model.firms.D_i .+= DD_i

model.firms.L_i .= (1 - prop.theta) * model.firms.L_i + DL_i

model.firms.E_i .= Bit.equity_firms(model.firms, prop.a_sg, model.agg.P_bar_g,
                                    model.agg.P_bar_CF)
...
zero = sum(data.nominal_gdp - data.nominal_household_consumption -
...
zero = sum(data.real_gdp - data.real_household_consumption -
