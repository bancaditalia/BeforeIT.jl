...
function initialize_variables_retail_market(firms, rotw, prop, agg, w_act, w_inact, gov, bank)
    ...
    Q_d_i_g = zeros(Float64, size(firms.Y_i)..., G)
    Q_d_m_g = zeros(Float64, size(rotw.Y_m)..., G)

    C_h = zeros(Float64, H)
    I_h = zeros(Float64, H)

    C_j_g = zeros(Float64, 1, G)
    C_l_g = zeros(Float64, 1, G)

    P_bar_h_g = zeros(Float64, 1, G)
    P_bar_CF_h_g = zeros(Float64, 1, G)

    P_j_g = zeros(Float64, 1, G)
    P_l_g = zeros(Float64, 1, G)
...
function initialize_variables_firms_market(firms, rotw, prop)
    ...
    I_i_g = zeros(Float64, I, G)         # output
    P_CF_i_g = zeros(Float64, I, G)
    DM_i_g = zeros(Float64, I, G)
    P_bar_i_g = zeros(Float64, I, G)
...
function perform_firms_market!(
    ...
    DM_nominal_ig = zeros(Float64, size(DM_d_ig))
...
function perform_retail_market!(
    ...
    C_real_hg = zeros(Float64, size(C_d_hg))
