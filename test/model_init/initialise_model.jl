...
@test isapprox(sum(model.firms.Pi_i), 16805.143545982424, atol = 1.0e-7)
@test isapprox(sum(model.firms.D_i), 54049.00000000002, atol = 1.0e-7)
@test isapprox(sum(model.firms.Y_i), 134635.75553779324, atol = 1.0e-7)
@test isapprox(sum(model.firms.N_i), 3866.0, atol = 1.0e-7)
