...
function generate_dm_test_comparison(
        error1, error2, rmse1, rmse2, horizons, number_variables)
...
function create_bias_rmse_tables_abm(forecast, actual, horizons, type, number_variables)
    ...
    rmse_comparison_data = generate_dm_test_comparison(
        error_abm, error_var, rmse_abm, rmse_var, horizons, number_variables)
    write_latex_table("rmse_$(type_prefix)abm.tex", country, rmse_comparison_data, horizons)

    bias_data = generate_mz_test_bias(
        error_abm, actual, bias_abm, horizons, number_variables)
...
function create_bias_rmse_tables_var(forecast, actual, horizons, type, number_variables, k)
    ...
    if k == 1
        ...
        write_latex_table(
            "rmse_$(type_prefix)var.tex", country, string.(input_data_rmse), horizons)

        bias_data = generate_mz_test_bias(
            error_var, actual, bias_var, horizons, number_variables)
        ...
        rmse_base_var, _, error_base_var = calculate_forecast_errors(
            forecast_base_var, actual)

        rmse_comparison_data = generate_dm_test_comparison(
            error_var_k, error_base_var, rmse_var_k,
            rmse_base_var, horizons, number_variables)
        write_latex_table(
            "rmse_$(type_prefix)var_$(k).tex", country, rmse_comparison_data, horizons)
    end
end
