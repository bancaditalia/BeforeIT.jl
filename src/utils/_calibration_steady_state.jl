...
function get_params_and_initial_conditions_steady_state(
    ...
    firm_interest = 0.0 # STEADY-STATE
    ...
    interest_government_debt = 0.0 # STEADY-STATE 
    ...
    government_deficit = 0.0 # STEADY-STATE
    ...
    employers_social_contributions = min(
        social_contributions, sum(compensation_employees) - wages)
    ...
    timescale = data["nominal_gdp_quarterly"][T_calibration_exo] / (
        sum(
        compensation_employees .+ operating_surplus .+ capital_consumption .+
        taxes_production .+
        taxes_products,
    ) .+ taxes_products_household .+ taxes_products_capitalformation_dwellings .+
    ...
    tau_FIRM = timescale * corporate_tax / (
        sum(
        Bit.pos(
    ...
    alpha_Y_EA = 1.0 # STEADY-STATE
    beta_Y_EA = 0.0 # STEADY-STATE
    sigma_Y_EA = 0.0 # STEADY-STATE
    ...
    xi_gamma = 0.0 # STEADY-STATE
    pi_star = 0.0  # STEADY-STATE
    ...
    alpha_G = 1.0 # STEADY-STATE
    beta_G = 0.0  # STEADY-STATE
    sigma_G = 0.0 # STEADY-STATE
    alpha_E = 1.0 # STEADY-STATE
    beta_E = 0.0  # STEADY-STATE
    sigma_E = 0.0 # STEADY-STATE
    alpha_I = 1.0 # STEADY-STATE
    beta_I = 0.0  # STEADY-STATE
    sigma_I = 0.0 # STEADY-STATE
    C = zeros((3, 3)) # STEADY-STATE
    ...
    Y = timescale * sum(output) .* ones(T_calibration_exo - T_estimation_exo + 1, 1) # STEADY-STATE
    pi = zeros(T_calibration_exo - T_estimation_exo + 1, 1)   # STEADY-STATE
    ...
    C_E = [timescale *
           ...
           data["real_exports_quarterly"][T_estimation_exo:min(
               T_calibration_exo + T, T_calibration_exo_max)] /
           data["real_exports_quarterly"][T_calibration_exo]
           fill(NaN, max(0, T_calibration_exo + T - T_calibration_exo_max), 1)]
    Y_I = [timescale *
           sum(imports) *
           data["real_imports_quarterly"][T_estimation_exo:min(
               T_calibration_exo + T, T_calibration_exo_max)] /
           data["real_imports_quarterly"][T_calibration_exo]
           fill(NaN, max(0, T_calibration_exo + T - T_calibration_exo_max), 1)]
